...
def create_inputs(imgs, im_info):
    ...
    if len(imgs) == 1:
        ...
        prpoposals = selective_search(inputs['image'][0].transpose((1,2,0)), scale=1.0, sigma=0.8, min_size=50).astype('float32')
        prpoposals = prpoposals[:, :4]
        rois_num = 1000 if 1000 < len(prpoposals) else len(prpoposals)
        # np.array((rois_num, )).astype("int32")
        inputs["rois_num"] = np.array((rois_num, )).astype("int32")
        inputs["proposal_box"] = prpoposals[:rois_num, :]

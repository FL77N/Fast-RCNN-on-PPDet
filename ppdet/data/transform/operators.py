...
class RandomFlip(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            if sample["proposal_box"] is not None and len(sample["proposal_box"]) > 0:
                sample["proposal_box"] = self.apply_bbox(sample["proposal_box"], width)
...
class Resize(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if sample["proposal_box"] is not None and len(sample["proposal_box"]) > 0:
            sample["proposal_box"] = self.apply_bbox(sample["proposal_box"],
                                                    [im_scale_x, im_scale_y],
                                                    [resize_w, resize_h])

...
class COCODataSet(DetDataset):
    ...
    def parse_dataset(self):
        ...
        if self.pro_path is not None:
            pro_file = open(self.pro_path, "rb")
            pro_info = pickle.load(pro_file)
        ...
        for img_id in img_ids:
            ...
            if not self.load_image_only:
                ...
                proposal_box = gt_bbox

                if self.pro_path is not None:
                    proposal_box = pro_info["boxes"][pro_info["ids"].index(img_id)]
                    topk = self.pro_topk if self.pro_topk < proposal_box.shape[0] else proposal_box.shape[0]
                    proposal_box = proposal_box[:topk, :]
